    ...
    <div className="evnsv-container">
      ...
      <div className="evnsv-tabs">
        {[
          ...
          <button
            key={key}
            className={`evnsv-tab ${activeTab === key ? 'active' : ''}`}
      ...
      <div className="evnsv-chiprow">
        <button className={`evnsv-chip ${isPublic ? 'on' : ''}`} onClick={() => setIsPublic(true)}>Public</button>
        <button className={`evnsv-chip ${!isPublic ? 'on' : ''}`} onClick={() => setIsPublic(false)}>Private</button>
        <div className="evnsv-icons">
          <button className="evnsv-icbtn" aria-label="filter">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="22 3 2 3 10 12 10 19 14 21 14 12 22 3"></polygon></svg>
          </button>
          <button className="evnsv-icbtn" aria-label="sort">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h13"></path><path d="M3 12h9"></path><path d="M3 18h5"></path><path d="M19 6v12"></path><path d="M22 9l-3-3-3 3"></path></svg>
          </button>
              ...
              <div className="evnsv_bottomrow">
                <div className="evnsv_rating">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <svg key={i} xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill={i < st.rating ? '#FFC107' : 'none'} stroke="#FFC107" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15 8.5 22 9 17 13.5 18.5 20 12 16.5 5.5 20 7 13.5 2 9 9 8.5 12 2"></polygon></svg>
                  ))}
                </div>
