    ...
    <div className="bc-container">
      ...
      <div className="bc-grid">
        {filteredCars.map((car) => (
          <div key={car.id} className="bc-card">
            {car.new && <span className="bc-newBadge">NEW</span>}
            <img src={car.img} alt={car.name} className="bc-carImg" />
            <div className="bc-carName">{car.name}</div>
            <div className="bc-rating">⭐ {car.rating.toFixed(1)}</div>
            <div className="bc-cardFooter">
              <div className="bc-price">${car.price.toLocaleString()}</div>
              <button
                onClick={() => toggleFavorite(car.id)}
                className="bc-favoriteBtn"
              >
                <Heart
                  size={18}
                  className={
                    favorites.includes(car.id) ? "bc-heartActive" : "bc-heart"
                  }
                />
              </button>
            </div>
